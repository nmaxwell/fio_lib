%function u = bfio_eval(N,SL,EL,EXT,EPS,fun,f,mats,dir)
...
% [mats,dir] = bfio_prep(EL,EPS);
  ...
  zB = N/nz; %number of k in each zone
  
  for z1=0:nz-1 %loop over zones
    for z2=0:nz-1
      fprintf(1,'%d %d\n', z1,z2);
      k1stt = z1*zB; %real k at start and end of zones
      k1end = (z1+1)*zB;
      k2stt = z2*zB;
      k2end = (z2+1)*zB; 
      
      NOW = [];
      for ell=SL:-1:ML % start at 'top' of tree, work way down to middle
        nk = 2^ell; % number of k-branches
        nx = N/nk; % 
        kB = N/nk; % number of k-points in current branch
        xB = 1/nx; % 
        ...
        % z1*zB/(N/nk) = 2^ell*z1/nz
        
        for k1=(k1stt/kB):(k1end/kB-1) 
          for k2=(k2stt/kB):(k2end/kB-1)
            
            kc1 = (k1+1/2)*kB - N/2; % center
            kc2 = (k2+1/2)*kB - N/2;
            
            for x1=0:nx-1
              for x2=0:nx-1
                xc1 = (x1+1/2)*xB; % center
                xc2 = (x2+1/2)*xB;
                %--------------
                ...
                if(ell==SL)
                  ...
                  sp1 = (so1+k1)*kB - N/2;
                  sp2 = (so2+k2)*kB - N/2;
                  src = [sp1(:)'; sp2(:)']; %the coordinate pairs in the source domain in k, was columns in a 2x? matrix
                  scl = fun(N,trg,src); %evaluating kernel at the x's in trg, and the k's in src
                  scl = reshape(scl,kB,kB); % ???
                  all = all.*scl; % OK
                  
                  %transform %  interpolating
                  all = dir*all*dir';
